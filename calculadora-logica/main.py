...
def procesar_proposicion():
    proposicion = request.form["formula"]
    resultado = request.form["formula"]

    valores = request.form["valores"]
    variables = request.form["formula"]

    listaValores = [True if i == "V" else False for i in valores]
    ...
    simbolos_dict = {nombre: simbolo for nombre, simbolo in zip(variables, listaValores)}
    expr = parse_expr(resultado)
    resultValor = expr.subs(simbolos_dict)
    ...
    if expresion is not None:
        ...
        print(resultado)
        print(simbolos_dict, resultado, resultValor, response)
        return {"response": response, "test": "V" if resultValor == True else "F"}
    else:
